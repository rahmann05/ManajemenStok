...
public class StokKeluarController {
    ...
    @FXML private Label lblOutputHariIni;
    @FXML private Label lblTerakhirKeluar;
    ...
    private void refreshData() {
        ...
        Map<String, String> stats = stokDAO.getRingkasanKeluarHariIni();

        if (lblOutputHariIni != null) {
            int totalTransaksi = Integer.parseInt(stats.getOrDefault("total_transaksi", "0"));
            lblOutputHariIni.setText(totalTransaksi + " Transaksi");
        }

        if (lblTerakhirKeluar != null) {
            lblTerakhirKeluar.setText(stats.getOrDefault("terakhir_keluar", "-"));
        }
        ...
        if (tblTransaksiHariIni != null) {
            ...
            tblTransaksiHariIni.setItems(stokDAO.getTransaksiKeluarHariIni());
        }
    }
    ...
    private void handleSimpan() {
        ...
        try {
            ...
            if (jumlah > selected.getStokSaatIni()) {
                new Alert(Alert.AlertType.WARNING, "Stok tidak mencukupi! Sisa stok: " + selected.getStokSaatIni() + " " + selected.getSatuan()).show();
                return;
            }

            boolean success = stokDAO.simpanStokKeluar(selected, jumlah, datePicker.getValue(), txtKeterangan.getText());
