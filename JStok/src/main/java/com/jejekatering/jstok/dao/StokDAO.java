...
public class StokDAO {
    ...
    public Map<String, String> getRingkasanHariIni() {
        ...
        String queryTransaksi = "SELECT COUNT(*) as total_transaksi FROM transaksi_stok WHERE jenis_transaksi = 'Masuk' AND CAST(tanggal AS DATE) = CAST(NOW() AS DATE)";

        String queryTerakhir = """
            SELECT b.nama_bahan, t.jumlah, b.satuan 
            FROM transaksi_stok t 
            JOIN bahan b ON t.id_bahan = b.id_bahan 
            WHERE t.jenis_transaksi = 'Masuk' AND CAST(t.tanggal AS DATE) = CAST(NOW() AS DATE)
        ...
        try (Connection conn = KonekDB.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(queryTerakhir);
                ...
                if (rs.next()) {
                    ...
                    stats.put("terakhir_masuk", nama + " (" + jumlah + " " + satuan + ")");
                } else {
                    stats.put("terakhir_masuk", "-");
                }
            }
    ...
    public static class TransaksiHariIni {
        ...
        private final javafx.beans.property.SimpleStringProperty namaBahan;
        private final javafx.beans.property.SimpleStringProperty jumlah;
        private final javafx.beans.property.SimpleStringProperty oleh;
